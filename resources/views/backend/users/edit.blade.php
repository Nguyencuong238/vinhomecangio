                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            <label>{{ __('Name') }}:</label>
                            <input type="text" name="name" value="{{ old('name', $user->name) }}" class="form-control @error('name')is-invalid @enderror" placeholder="">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="email" value="{{ old('email', $user->email) }}" class="form-control @error('email')is-invalid @enderror" placeholder="">
                            @error('email')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select multiple="multiple" class="form-control form-control-select2" name="roles[]">
                                @foreach($roles as $role)
                                    <option {{ in_array($role->name, old('roles', $user->roles()->pluck('name')->toArray())) ? 'selected' : null }} value="{{ old('roles', $role->name) }}">{{ $role->name }}</option>
                                @endforeach
                            </select>
                        </div>
